...
start_zoo() {
    for Z in `seq 1 $ZOOS`; do
        ...
        local ZOO_RES=$(zoo$Z/bin/zkServer.sh start)
        if [[ $? -ne 0 || ! -z $(grep "FAILED" <<< "$ZOO_RES") ]]; then
            >&2 echo "Error: Unable to start ZooKeeper in $(pwd)/zoo${Z}: $ZOO_RES"
            >&2 echo "Please inspect logs in $(pwd)/zoo${Z}/logs/"
            exit 8
        fi
...
start_solr() {
    ...
    for S in `seq 1 $SOLRS`; do
        ...
        SOLR_START_COMMAND="`pwd`/solr$S/bin/solr -m $SOLR_MEM -cloud -s $LOCAL_SHOME -p $SOLR_PORT -z $HOST:$ZOO_BASE_PORT -h $HOST"
