...
function zoo() {
    ...
    for Z in `seq 1 $ZOOS`; do
        ...
        if [ ! -s "$ZCONF" ]; then
            ...
            # TODO: Disabling does not work. Fix it!
            echo "admin.enableServer=false" >> "$ZCONF"
            echo "admin.serverPort=$(( ZPORT + 10))" >> "$ZCONF"
        else
...
function solr() {
    ...
    # Add support for Danish ICU (sorting and normalising)
    if [[ "9." == $(grep -o "^.." <<< "$VERSION") ]]; then
        echo "Copying Solr 9 support jars"
        cp -v ${FOLDER}/modules/analysis-extras/lib/*.jar ${FOLDER}/server/solr-webapp/webapp/WEB-INF/lib/
        echo "Change binding from localhost to public"
        sed -i 's/#SOLR_JETTY_HOST="127.0.0.1"/SOLR_JETTY_HOST="0.0.0.0"/' "${FOLDER}/bin/solr.in.sh"
    elif [ "." == ".`echo \" $LAYOUT2_VERSIONS \" | grep \" $DEST \"`" ]; then
        echo "Injecting support WARS to solr.war"
        # We inject the JARs into the WAR to avoid fiddling with solrconfig.xml
        pushd $FOLDER/example/webapps/ > /dev/null
        if [[ $? -ne 0 ]]; then
            >&2 echo "Error: Expected folder missing. PWD=$(pwd)"
            exit 7
        fi
