...
<xsl:transform xmlns:f="http://www.w3.org/2005/xpath-functions"
                ...
                <xsl:variable name="errorDoc">
                    <f:map>
                        ...
                        <f:map key="kb:internal">
                            ...
                            <f:string key="kb:transformation_error">
                                <xsl:value-of select="true()"/>
                            </f:string>
                            <f:string key="kb:transformation_error_description">
            ...
            <xsl:try>
                ...
                <xsl:for-each select="$pbCore">
                                        ...
                                        <xsl:choose>
                                            ...
                                            <xsl:when test="contains(./contributor, ':')">
                                                <f:map key="actor">
                                                    ...
                                                    <f:string key="name">
                                                        <xsl:value-of
                                                                select="normalize-space(substring-after(./contributor, ':'))"/>
                                                    </f:string>
                                                </f:map>
                                                <f:string key="characterName">
                                                    <xsl:value-of
                                                            select="normalize-space(substring-before(./contributor, ':'))"/>
                                                </f:string>
                                            </xsl:when>
                    ...
                    <xsl:choose>
                        ...
                        <xsl:when
                                test="normalize-space($pbCore/pbcoreInstantiation/formatAspectRatio) = '16:9,' or normalize-space($pbCore/pbcoreInstantiation/formatAspectRatio) = '16:9, '">
                            <f:string key="videoFrameSize">16:9</f:string>
                        </xsl:when>
                        <xsl:when
                                test="$pbCore/pbcoreInstantiation/formatAspectRatio != '' and normalize-space($pbCore/pbcoreInstantiation/formatAspectRatio) != ',' or ', '">
    ...
    <xsl:template name="pbc-metadata">
                ...
                <xsl:for-each select="./pbcoreExtension/extension">
                    <xsl:if test="f:contains(., 'live:live') or f:contains(., 'live:ikke live')">
                        <f:boolean key="isLiveBroadcast">
                            ...
                            <xsl:choose>
                                <xsl:when test="contains(., 'live:live')">
                                    <xsl:value-of select="f:true()"/>
                                </xsl:when>
                                <xsl:when test="contains(., 'live:ikke live')">
        ...
        <xsl:variable name="produktionslandBoolean">
            <xsl:choose>
                <xsl:when
                        test="$pbcExtensions[f:contains(., 'produktionsland:') or f:contains(., 'produktionsland_id:')]">
                    <xsl:value-of select="f:true()"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="false()"/>
        ...
        <xsl:if test="$produktionslandBoolean = f:true()">
            <f:map key="countryOfOrigin">
                ...
                <xsl:for-each select="./pbcoreExtension/extension">
                    <xsl:choose>
                        <xsl:when
                                test="f:contains(. , 'produktionsland:') and substring-after(. , 'produktionsland:') != ''">
                            <f:string key="name">
                                <xsl:value-of select="f:substring-after(. , 'produktionsland:')"/>
                            </f:string>
                        </xsl:when>
                        <xsl:when
                                test="f:contains(. , 'produktionsland_id:') and substring-after(. , 'produktionsland_id:') != ''">
        ...
        <xsl:if test="$pbcExtensions[f:contains(., 'premiere:premiere')] and ./pbcoreInstantiation/pbcoreDateAvailable/dateAvailableStart">
            <f:string key="datePublished">
                <xsl:value-of
                        select="f:substring-before(pbcoreInstantiation/pbcoreDateAvailable/dateAvailableStart, 'T')"/>
            </f:string>
        </xsl:if>
                        ...
                        <xsl:otherwise>
                          ...
                          f:string-length(substring-after(., 'antalepisoder:')) > 0">
                                    ...
                                    <f:map key="partOfSeason">
                                        ...
                                        <xsl:variable name="numberOfEpisodes">
                                            <xsl:value-of
                                                    select="number(normalize-space(substring-after(., 'antalepisoder:')))"/>
                                        </xsl:variable>
                                        <xsl:if test="string($numberOfEpisodes) != 'NaN'">
                                            <f:number key="numberOfEpisodes">
        ...
        <xsl:choose>
            <xsl:when test="//pbcoreGenre">
                ...
                <xsl:variable name="Music" as="item()*"
                              select="('musik', 'kor- og orkestervirksomhed', 'opera', 'rytmisk musik', 'populær musik', 'popmusik', 'populær musik popmusik')"/>
                ...
                <xsl:variable name="ChildrenYouth" as="item()*"
                              select="('børn &amp; ungdom', 'dyr med central rolle', 'eventyr')"/>
                ...
                <xsl:variable name="keywordsSequence" as="item()*">
                    <xsl:for-each select="./pbcoreGenre/genre">
                        <xsl:choose>
                            <xsl:when
                                    test="f:contains(., ':') and substring-after(., ':') != '' and not(f:contains(., 'null'))">
                ...
                <xsl:choose>
                    <xsl:when test="$keywordsString != ''">
                        ...
                        <xsl:variable name="genreValue">
                            <xsl:choose>
                                <xsl:when
                                        test="my:sequenceAContainsValueFromSequenceB($keywordsSequence, $NewsPoliticsSociety)">
                                ...
                                <xsl:when
                                        test="my:sequenceAContainsValueFromSequenceB($keywordsSequence, $Entertainment)">
                                    <xsl:value-of select="'Humor, quiz og underholdning'"/>
                                </xsl:when>
                                <xsl:when
                                        test="my:sequenceAContainsValueFromSequenceB($keywordsSequence, $ChildrenYouth)">
                                    <xsl:value-of select="'Børn og unge'"/>
                                </xsl:when>
                                <xsl:when
                                        test="my:sequenceAContainsValueFromSequenceB($keywordsSequence, $Documentary)">
                                ...
                                <xsl:when
                                        test="my:sequenceAContainsValueFromSequenceB($keywordsSequence, $ScienceNature)">
        ...
        <f:array key="identifier">
            ...
            <xsl:if test="normalize-space($productionId) != ''">
                <f:map>
                    ...
                    <f:string key="value">
                        <xsl:value-of select="normalize-space($productionId)"/>
                    </f:string>
            ...
            <xsl:if test="//pbcoreIdentifier">
                <xsl:for-each select="./pbcoreIdentifier">
                    <xsl:choose>
                        ...
                        <xsl:when
                                test="identifierSource = 'Det Kongelige Bibliotek; Radio/TV-samlingen; De hvide programmer'">
                            <f:map>
                                <f:string key="@type">PropertyValue</f:string>
            ...
            <xsl:if test="$InternalAccessionRef != ''">
                <f:map>
                    ...
                    <f:string key="value">
                        <xsl:value-of select="$InternalAccessionRef"/>
                    </f:string>
                </f:map>
            </xsl:if>
        </f:array>
    ...
    <xsl:template name="kb-internal">
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="/XIP/Metadata/Content/program_structure">
                        ...
                        <xsl:for-each select="/XIP/Metadata/Content/program_structure">
                            <xsl:choose>
                                <xsl:when test="holes/hole/holeLength[text() &gt; $maxMissingSeconds]">
                                    <xsl:value-of select="false()"/>
                                </xsl:when>
                                <xsl:when test="missingStart/missingSeconds[text() &gt; $maxMissingSeconds]">
                                    <xsl:value-of select="false()"/>
                                </xsl:when>
                                <xsl:when test="missingEnd/missingSeconds[text() &gt; $maxMissingSeconds]">
                                    <xsl:value-of select="false()"/>
                                </xsl:when>
                                <xsl:when test="holes/hole/holeLength[text() &gt; $maxMissingSeconds]">
                                    <xsl:value-of select="false()"/>
        ...
        <xsl:if test="/XIP/Metadata[@schemaUri = 'http://id.kb.dk/schemas/radiotv_access/transcoding_status']">
            ...
            <xsl:variable name="filePath">
                <xsl:value-of
                        select="/XIP/Metadata/Content/radiotvTranscodingStatus/specificRadioTvTranscodingStatus/accessFilePath"/>
            </xsl:variable>
        ...
        <xsl:choose>
            <xsl:when test="$pbCore/pbcoreInstantiation/formatChannelConfiguration = 'surround'">
                <f:boolean key="kb:surround_sound">
                    <xsl:value-of select="true()"/>
                </f:boolean>
            </xsl:when>
    ...
    <xsl:template name="extension-extractor">
        ...
        <xsl:choose>
            ...
            <xsl:when
                    test="f:starts-with(extension , 'kanalid:') and f:string-length(normalize-space(substring-after(extension , 'kanalid:'))) > 0">
