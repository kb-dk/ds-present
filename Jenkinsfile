pipeline {
    ...
    parameters {
        ...
        string(name: 'ORIGINAL_JOB', defaultValue: "ds-present", description: 'What job was the first to build?')
    ...
    stages {
                    ...
                    withMaven(options: [artifactsPublisher(fingerprintFilesDisabled: true, archiveFilesDisabled: true)], traceability: true, mavenLocalRepo: "${WORKSPACE}/repository") {
                        switch (params.ORIGINAL_JOB) {
                            ...
                            case ['ds-license']:
                                ...
                                echo "Changing MVN dependency ds-license to: ${params.ORIGINAL_BRANCH}-${params.ORIGINAL_JOB}-${params.ORIGINAL_JOB}-SNAPSHOT"
                                break
                        }
                    }
                }
        ...
        stage('Trigger ds-kaltura Build') {
            when {
                expression {
                    currentBuild.currentResult == "SUCCESS" && params.ORIGINAL_BRANCH ==~ "master|release_v[0-9]+|PR-[0-9]+"
                }
