...
## [4.0.2](https://github.com/kb-dk/ds-present/releases/tag/ds-present-4.0.2) 2026-03-25
### Added
- Solr field for annotations
- Bumb solr schema version to 1.8.7

## [4.0.1](https://github.com/kb-dk/ds-present/releases/tag/ds-present-4.0.1) 2026-03-04

### Fixed

- Handle datetime with milliseconds using OffsetDateTime
...
- update of solr config version in pom file

## [3.0.1](https://github.com/kb-dk/ds-present/releases/tag/ds-present-3.0.1) 2025-09-01

### Changed

- add title_restricted field to solr when indexing

## [3.0.0](https://github.com/kb-dk/ds-present/releases/tag/ds-present-3.0.0) 2025-06-12

### Added

- Integration unittest with OAuth access token. Require kb-util v.1.6.10
- Added solr fields file_path, nielsen_channel_id and ritzau_channel_id

### Changed

- Removed autogenerated client class. Now uses the manual maintained client only.
- Solr version bumped to v. 1.7.11
- Solr version bumped to v. 1.8.0
- Moved ForbiddenServiceException to kb-util. Now requires kb-util v. 1.6.10-SNAPSHOT
...
- IIIF metods not supported by service2service Oauth since no java client is generated for these methods. Also not
...
- Fixed an issue where holdback name for records with the holdback value for "Undervisning" did not get populated
  correctly.
...
- Added functionality to change own production threshold in configuration files, with a default value which includes
  own-, co- and enterprise-production for DR records.

### Changed

- Renamed solr fields `own_production` and `own_production_code` to `production_code_allowed` and
...
- Changed configuration naming for YAML key `holdback.dr` to `dr.holdback` in preparation for addition of
  `dr.restrictions`.
...
- solr 1.7.4 : New field type as text_general but without stopwords. The title field now uses this field instead. It
  needed for better title match.
...
- Fixed cases where transformation errors wasn't handled correct. Now all errors gets written to the transformed
  documents.
...
- Changed field used for ownproduction calculation from country_of_origin to origin as specified by DR metadata
  specialists.
...
- Some documents did not get the channel facet correctly created. A check for these DR1 and DR2 records have been added
  in the preservica2schemaorg XSLT.
...
- Added the query used to build frontpage of the webapp to warmup of solr searchers. Lowering Qtime from approx 300 ms
  to 50 ms on a local setup.
...
- Changed how values are extracted from Preservica records. Now all needed values are extracted at once, saving
  iterations on the XML stream.
...
- Updated XSLTs to handle edge cases, where multiple 'hovedgenre' are present in data. If multiple are present, the
  shortest string is used as genre in the schema.org JSON.
...
- Update logging in HoldbackDatePicker class making it clearer, that origins in a holdback context comes from TVMeter
  data.
...
- Added View strategy for Preservica 7, so that manifestations can be extracted correctly for Preservica 5 and
    7. [DRA-662](https://kb-dk.atlassian.net/browse/DRA-662)
...
- Added solr fields containing start and end dates as strings in Danish local
  time. [DRA-599](https://kb-dk.atlassian.net/browse/DRA-599)
...
- Removed spellcheck.maxCollationTries from spellcheck component. It crashed Solr on core reload (after configuration
  changes)
...
- Spellcheck compontent added to Solr search. If no results are found, the solr reply will come with query suggestions
...
- Added ReflectUtil class containing methods used for testing. Using these methods instead of internal mockito methods
  makes the project not depend on mockitos old internal class.
